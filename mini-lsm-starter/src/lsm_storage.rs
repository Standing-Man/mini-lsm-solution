...
impl LsmStorageInner {
    ...
    pub fn get(&self, key: &[u8]) -> Result<Option<Bytes>> {
        ...
        if let Some(v) = state.memtable.get(key) {
            return Ok((!v.is_empty()).then(|| v));
        }
        for mem_table in &state.imm_memtables {
            if let Some(v) = mem_table.get(key) {
                return Ok((!v.is_empty()).then(|| v));
            }
        }
        Ok(None)
    ...
    pub fn put(&self, key: &[u8], value: &[u8]) -> Result<()> {
        ...
        if state.memtable.approximate_size() >= self.options.target_sst_size {
            // release the read guard in advance
            drop(state);
    ...
    pub fn scan(
        ...
        let lsm_iter = LsmIterator::new(MergeIterator::create(iters)).unwrap();
